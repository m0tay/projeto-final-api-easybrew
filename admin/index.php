...
$inactive_machines = count(array_filter($machines_result['records'] ?? [], fn($m) => empty($m['is_active'])));
...
// como era bom um PHP 8.5 aqui com o novo operador |>
// $recent_transactions |> array_column($$, 'user_id') |> array_filter($$) |> array_unique($$)
$user_ids = array_unique(array_filter(array_column($recent_transactions, 'user_id')));
$users_map = [];
foreach ($user_ids as $uid) {
  $user = callAPI('users/read.php', ['id' => $uid]);
  if ($user) {
    $users_map[$uid] = $user;
  }
}
  ...
  <div class="col-xl-3 col-md-6">
    <div class="card bg-danger text-white mb-4">
      <div class="card-body">
        <div class="d-flex justify-content-between align-items-center">
          <div>
            <div class="small">Máquinas Inoperantes</div>
            <div class="fs-2"><?= $inactive_machines ?></div>
          </div>
    ...
    <table class="table table-striped">
      ...
      <tbody>
        <?php foreach ($recent_transactions as $t): ?>
          <tr>
            <td><?= h($t['id']) ?></td>
            <td><?= h($users_map[$t['user_id']]['first_name'] ?? 'N/A') ?></td>
